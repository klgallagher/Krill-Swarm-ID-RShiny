...
server <- function(input, output) {
  ...
  observeEvent(input$go, {
      if(input$glider == 'UD'){
          #these color bars were generated using the colscatter function, 
        #provided above the app script.
        #This function takes a column of data and provides a list of 
          load('/path/to/colorbar_UD.RData')
          r$col.bar <- col.bar
      } else {
          load('/path/to/colorbar_UAF.RData')
          r$col.bar <- col.bar
      }
  })
